...
function createInitialBotState(): WidgetBotState {
  return {
    ...
    messages: [
      {
        from: "bot",
        text: "Hola, soy el asistente virtual de TravelBot. ¿Cuál es tu nombre?",
      },
    ],
  };
}
...
export function DemoWebView({
  ...
  const sendInput = () => {
    ...
    if (bot.step === "ask_ages") {
      ...
      const quote: QuoteItem = {
        ...
        destino: pkg.destino,
        hotel: pkg.hotel,
        pasajeros: Number(bot.draft.pasajeros || 1),
        edades: value,
        precioBase: pkg.precio,
        total: Number(bot.draft.pasajeros || 1) * pkg.precio,
        estado: "Enviada",
      };
      ...
      addBotMessage(
        `Excelente ${bot.nombre}. Te enviamos el presupuesto ${quote.numero} para ${pkg.destino} por ${formatUsd(
          quote.total
        )}.`
      );
      return;
  ...
  return (
    <div className="relative min-h-[78vh] overflow-hidden rounded-[28px] border border-slate-200 bg-white">
      <header className="border-b border-slate-200 bg-white px-6 py-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="text-2xl font-bold text-slate-900">LifeTrip Travel</div>
            <div className="text-sm text-slate-500">
              Agencia demo con asistente virtual integrado
            </div>
          </div>
      ...
      <section className="bg-gradient-to-r from-sky-500 to-cyan-500 px-6 py-12 text-white">
        <div className="max-w-4xl">
          ...
          <h2 className="mt-4 text-4xl font-bold leading-tight">
            Descubrí tu próximo viaje con asistencia personalizada
          </h2>
          <p className="mt-3 max-w-2xl text-white/90">
            Paquetes, hoteles, pasajes y excursiones en un solo lugar, con ayuda inmediata desde nuestro asistente virtual.
          </p>
        </div>
      </section>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {quickActions.map((item) => {
              ...
              return (
                <button
                  key={item.label}
                  className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-medium text-slate-700 hover:bg-sky-50"
                >
      ...
      <footer className="border-t border-slate-200 px-6 py-5 text-sm text-slate-500">
        © 2026 LifeTrip Travel · Demo comercial para presentación
      </footer>

      {!widgetOpen && (
        <button
          onClick={() => setWidgetOpen(true)}
          className="fixed bottom-6 right-6 flex items-center gap-3 rounded-full bg-white px-4 py-3 shadow-xl ring-1 ring-slate-200 hover:bg-slate-50"
        >
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-sky-100 text-3xl">
            🤖
          </div>
          <div className="text-left">
            <div className="text-sm font-semibold text-slate-900">Asistente virtual</div>
            <div className="text-xs text-slate-500">Estamos para ayudarte</div>
          </div>
        </button>
      )}
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[360px] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl">
          <div className="flex items-center justify-between bg-sky-500 px-4 py-3 text-white">
            <div>
              ...
              <div className="text-xs text-white/80">TravelBot Web</div>
            </div>
